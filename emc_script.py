...
def set_origin(obj, current_cam):
    # Check if a Child Of constraint already exists for the object
    ...
    # If a Child Of constraint exists, update its target
    if child_of_constraint:
        child_of_constraint.target = current_cam
    else:
        # If no Child Of constraint exists, add a new one
        constraint = obj.constraints.new(type='CHILD_OF')
        constraint.target = current_cam
        constraint.use_scale_x = False
        constraint.use_scale_y = False
        constraint.use_scale_z = False

        # Set the inverse correction for the Child Of constraint
...
def sync_blender_cam(addr):
    global previous_active_camera
    global cam
    global org
    # Get the active camera
    active_camera = bpy.context.scene.camera
    # Check if the active camera has changed
    if active_camera != previous_active_camera:
        previous_active_camera = active_camera
        cam = active_camera
...
class menu_img_sequence(bpy.types.Operator):
    ...
    def execute(self, context):
        scene = context.scene
        wm = context.window_manager
        self._timer = wm.event_timer_add(0.0167, window=context.window)
        wm.modal_handler_add(self)
        load_state_last()
        time.sleep(0.1)
...
class emc_control_panel(Panel):
    ...
    def draw(self, context):
        ...
        quick_state_box.enabled = draw_panel
